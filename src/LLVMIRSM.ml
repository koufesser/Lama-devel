  ...
  (*List.iter (function x -> print_string x) args;
    ...
    | LABEL s | FLABEL s -> 
      ...
      |Function x ->   
        ...
        x#add_label (new label_c s block)
      )
    | SLABEL s ->  ()
...
let build_one (insn : SM.insn) (prg : SM.prg) = 
  ...
  | CONST i ->
    ...
    let string_val = Llvm.build_global_stringptr s (get_name ()) builder in
    (* assert_in_scope (); *)
  ...
  | LD desig ->
      ...
      (* assert_in_scope (); *)
      (match desig with
      | Global s ->  let (value, val_type) = global_scope#get_global s in
       ignore @@ func#load value val_type 
      | Local i -> failwith "Load local failed" 
      | Arg i -> failwith "Load arg failed"
  ...
  | ST desig ->
      ...
      | Global s  -> let (value, val_type) = func#store in
                      global_scope#set_global s value val_type 
  ...
  | FLABEL s | LABEL s -> 
    ...
    | _ -> 
      ...
      last_block := Some label#get_block)
  ...
  | BEGIN (name, _, _, _, _, _) ->
    ...
    let func =  global_scope#get_function name in
  ...
  | CALL (func_name, arity, is_tail_call) ->
    ...
    ".array" ->
      let (value, value_type) = create_array arity in 
    ...
    let () =
      for _ = 1 to arity do
        ...
        args := value :: !args;
        args_type := Llvm.type_of value :: !args_type;
        func#drop
      done in 
    let args = Array.of_list !args in
    let function_type = Llvm.function_type i32_type @@ Array.of_list !args_type in
    let llvm_func = if global_scope#has_function
      func_name then LL.lookup_func_exn func_name 
      else 
      Llvm.declare_function func_name function_type main_module in
    let name =  get_name () in
  ...
  | RET ->
      ...
      let func = current_function () in
      ignore @@ Llvm.build_ret func#store builder;
      func#drop;
  ...
  | EXTERN str ->
    ...
    global_scope#add_extern_function str
  | PUBLIC str ->
