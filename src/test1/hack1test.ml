...
let log fmt =
  Format.kasprintf (fun s -> Format.printf "%s\n%!" s) fmt
...
let rec codegen_expr () =
    let lhs =
        ...
        lamaint_to_ptr (Llvm.const_int lama_int_type 52) in
    let rhs =
      match Base.Hashtbl.find named_values "x" with
        | None -> failwiths "unkown variable name %s" "x"
        (* Load the value *)
        | Some v ->
Llvm.build_load v "x" builder
        in

        log "adding '%s' and '%s'" (Llvm.string_of_llvalue lhs) (Llvm.string_of_llvalue rhs);
        Printf.printf " creating binop %s %d\n%!" __FILE__ __LINE__;
        lamaint_to_ptr (Llvm.build_add lhs rhs "addtmp" builder)

let%expect_test " _" =
  gen_func codegen_expr "foo" ["x"] ();
