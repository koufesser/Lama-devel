...
let pp_pattern fmt p = GT.transform Pattern.t (new pp_pattern) fmt p

class pp_expr on_decl fself =
  object
    inherit [Format.formatter, Expr.t, unit] Expr.t_t
    inherit [Expr.t] Expr.fmt_t_t (fself : Format.formatter -> Expr.t -> unit)
    method! c_Const ppf _ = Format.fprintf ppf "%d"
    ...
    method! c_If ppf _ c th el =
      Format.fprintf ppf
        "@[if %a then @[<v 2>{@,%a@]@ @[<v 2>} else {@,%a@]@ } fi@]" fself c
        fself th fself el
    ...
    method c_Repeat ppf (_ : Expr.t) cond body =
      Format.fprintf ppf "@[<v 2>";
    ...
    method! c_Case ppf _ scru cases _ _ =
      ...
      cases
      |> List.iteri (fun i (p, e) ->
             Format.fprintf ppf "@[%s %a@ ->@ %a@]@ "
               (if i = 0 then " " else "|")
               pp_pattern p fself e);
      Format.fprintf ppf "@]";
      Format.fprintf ppf "@[esac@]";
      Format.fprintf ppf "@]"

    method c_Return ppf (_ : Expr.t) e =
      match e with
      | None -> Format.fprintf ppf "return"
      | Some e -> Format.fprintf ppf "@[return@ %a@]" fself e

    method! c_Ignore ppf (_ : Expr.t) e = Format.fprintf ppf "@[%a@]" fself e
    method! c_Unit ppf (_ : Expr.t) = Format.fprintf ppf "Unit "
    ...
    method! c_Intrinsic ppf _ _ = Format.fprintf ppf "Intrinsic"
    method! c_Control ppf _ _ = Format.fprintf ppf "Control"
  end

class pp_decl _fself on_expr =
  object (self)
    inherit [string * Format.formatter, _, unit] Expr.decl_t

    method qualifier ppf : Expr.qualifier -> _ =
      function
      | `Local -> ()
    ...
    method c_DECL (name, ppf)
        ( qual,
          (item :
            [ `Fun of string list * Expr.t | `Variable of Expr.t GT.option ]) )
        =
      match item with
      | `Variable None -> Format.fprintf ppf "local %s;" name
      | `Variable (Some e) -> Format.fprintf ppf "local %s = %a;" name on_expr e
      | `Fun (ss, e) ->
          Format.fprintf ppf "@[<v>";
          Format.fprintf ppf "@[%afun %s (%a) @]@," self#qualifier qual name
            self#args ss;
...
let fix_decl decl0 t0 =
  let rec traitdecl inh subj = Expr.gcata_decl (decl0 traitdecl traitt) inh subj
  and traitt inh subj = Expr.gcata_t (t0 traitdecl traitt) inh subj in
  (traitdecl, traitt)

let pp fmt s = snd (fix_decl (new pp_decl) (new pp_expr)) fmt s

let pp ppf ast =
  let margin =
    try int_of_string @@ Sys.getenv "LAMA_MARGIN"
    with Failure _ | Not_found -> 80
  in
  Format.pp_set_geometry ppf ~margin ~max_indent:15;
  Format.printf "%a\n%!" pp ast
